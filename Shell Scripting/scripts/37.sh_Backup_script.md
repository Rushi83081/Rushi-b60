# 📦 Backup Script in AWS EC2 Instance

This project demonstrates how to create an **automated backup script** on an **AWS EC2 instance**, schedule it using **cron jobs**, and verify backup status using **log files**.

---

## 📄 1. Create Backup Script File

```bash
vim 37.sh
```

## 📜 2. Backup Script (37.sh)
```
#!/bin/bash

# Backup directory
backup_dir="backups"

# Source directory to backup
source_dir="/mnt/backup"

# Timestamp for unique backup name
timestamp=$(date "+%Y-%m-%d_%H-%M-%S")

# Backup file name
backup_file="$backup_dir/backup-$timestamp.tar.gz"

# Create backup directory if it does not exist
mkdir -p "$backup_dir"

# Create compressed backup
tar -czf "$backup_file" "$source_dir"

# Check backup status and log result
if [ $? -eq 0 ]
then
    echo "Backup Created Successfully: $backup_file" >> /var/log/backup.log
else
    echo "Backup Failed for $source_dir" >> /var/log/backup.log
fi
```

## 🧠 3. What We Did (Step-by-Step Explanation)
### 🔹 Step 1: Define Backup Variables

**Created variables for:**

-  📁 Backup directory

-  📂 Source directory

-  🕒 Timestamp

-  🗄️ Backup file name

✅ Timestamp ensures unique backups every time.

---
### 🔹 Step 2: Create Backup Directory

**Used `mkdir -p` to:**

-  📁 Create the backup folder if it doesn’t exist

-  🚫 Avoid errors if the folder already exists
---

### 🔹 Step 3: Create Compressed Backup

Used `tar -czf` to:

-  📦 Compress the source directory

-  🗜️ Store it as a `.tar.gz` file

-  🕒 Backup file includes date and time for easy tracking.
---

### 🔹 Step 4: Check Command Status

**Used `$?` to check whether the backup command succeeded or failed:**

-  ✅ 0 → Success

-  ❌ Non-zero → Failure
---

### 🔹 Step 5: Log Backup Result

**Logged the backup status in:**
```
/var/log/backup.log
```

### 🛠️ Helps in:

-  📊 Monitoring backups

-  🐞 Debugging failures

-  ⚙️ Automation tracking
---

## ⏰ 4. Cron Job Configuration

**Cron jobs are used to automate the execution of the backup script.**

### 🔹 Open Crontab Editor
```
crontab -e
```
### 🔹 Run Script at a Specific Time
```
34 05 * * * bash 37.sh
```
**➡️ Runs 37.sh every day at 05:34 AM.**

### 🔹 Run Script Every 2 Minutes
```
*/2 * * * * bash 37.sh
```
**➡️ Automatically takes backup every 2 minutes.**

---

## 📂 5. Check Backup Logs
### 🔹 Navigate to Log Directory
```
cd /var/log
```
### 🔹 View Backup Log File
```
cat backup.log
```
➡️ Used to verify whether backups are successful or failed.

---
## ⭐ 6. Key Concepts Used

-  🐚 Shell Scripting

-  📦 tar (archive & compression)

-  🕒 date command

-  📁 mkdir

-  📊 Exit status ($?)

-  📝 Logging

-  ⏰ Cron Jobs
---

## 🚀 7. Use Case

**-  Automated backups on AWS EC2**

**-  Regular data protection**

**-  DevOps & system administration automation**

**-  Easy monitoring using log files**
---
